...
class LocationEdit extends React.Component<Props, State> {
        ...
        <Container>
          ...
          <p>
            <Editable
              ...
              value={location.lat}
              onChange={value => this.update({ lat: parseFloat(value) })}
            />
            ,{' '}
            <Editable
              placeholder="Longitude"
              value={location.lon}
              onChange={value => this.update({ lon: parseFloat(value) })}
            />
          </p>
          <p>
            <Editable
  ...
  private submit() {
    syncer
      .updateLocation(this.props.location!.id, this.state.changes)
      .then(() => this.setState({ changes: {} }));
  }
}
